...
export default function SearchResults() {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      try {
        ...
        const allProperties = (await import('@/data/real-estate')).realEstateData as Array<RealEstate & { type: string }>;
  ...
  return (
    ...
    <div className={styles.container}>
      <SectionName title={`نتائج البحث عن: ${searchTerm || 'الكل'}`} subtitle={`${results.length} نتيجة`} className="mt-[7rem]"/>
      ...
      <form onSubmit={handleSearch} className={styles.searchForm}>
        <div className={styles.searchInputContainer}>
          <input
            ...
            aria-label="بحث"
          />
          <button type="submit" className={styles.searchButton} aria-label="بحث">
            <Search size={20} />
          </button>
        </div>
      </form>
